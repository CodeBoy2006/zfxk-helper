...
- State: populated local draft with three target rows; no live school account or running task
...
- Spacing and layout rhythm: the standalone page now matches the reference structure: dark top navigation, left task config, center group/target/teaching-class area, and right task status/events. The left task actions are visible in the first desktop viewport.
