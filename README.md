...
> 注意：当前自动登录系统仅在浙工大（浙江工业大学）的新版正方登录流程通过测试；其他院校可能因登录页、验证码、二次验证或认证网关差异而无法使用。遇到不兼容时，请改用已登录 Cookie。
...
* 不做全量课程搜索。
* 默认刷新间隔为 `1500ms`。
* 可以在选课入口开放前先启动任务；如果入口页暂时没有选课上下文，后台任务会按基础刷新间隔继续尝试初始化，直到入口开放或任务被取消。
...
通过 `bootstrapFromPage()` 初始化时，SDK 会沿用入口页路径中的 `gnmkdm` 请求 Display 面板；手动传入 HTML 时可在 `loadCourseTypeDisplayContext({ pagePath })` 中提供入口页路径。
